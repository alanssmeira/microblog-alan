...
function upload(array $arquivo){
     //Definindos os tipos de imagem aceitos
     $tiposValidos = [
        "image/png",
        "image/jpeg",
        "image/gif",
    ...
    /* Se o processo de envio temporário para destino for feito com sucesso, então a funçõa retorna verdadeiro (indicando o sucesso no processo) */
...
/* Usada em index.php */        // dispensa mais parâmetros pq é publica
function lerTodosOsPosts(mysqli $conexao):array {
    $sql = "SELECT id, imagem, titulo, resumo FROM posts ORDER BY data DESC";
...
function lerDetalhes(mysqli $conexao, int $idPost):array {    
    $sql = "SELECT posts.imagem, posts.titulo, posts.texto, posts.data, usuarios.nome AS autor FROM posts INNER JOIN usuarios ON posts.usuario_id = usuarios.id WHERE posts.id = $idPost";
    //INNER JOIN traz dados de 2 ou mais tabelas e precisa de critérios, no ON
...
function busca(mysqli $conexao, string $termo):array {
    ...
    // LIKE é ideal para buscas, permite encontrar a palavra dentro uma frase mas apenas se começar com a palavra buscada
    ...
    // A % (porcentagem) permite que a palavra seja encontrada em qualquer posição do texto    
